...
class UserUpdateForm(forms.ModelForm):
	...
	employee =  forms.ModelChoiceField(queryset=User.objects.all(),required=True,)
...
class EducationForm(forms.ModelForm):
	...
	specialization = forms.CharField(max_length=50, widget=forms.TextInput(attrs={'class': 'width-50 input-sm form-control',
		'required': 'True'}))
	from_date = forms.DateField(widget=DateTimePicker(options=dateTimeOption),)
	to_date = forms.DateField(widget=DateTimePicker(options=dateTimeOption),)
...
class PreviousEmploymentForm(forms.ModelForm):
	...
	#company_address = forms.CharField(max_length=50, widget=forms.TextInput(attrs={'class': 'width-50 input-sm form-control',
		#'required': 'True'}))
	employed_from = forms.DateField(widget=DateTimePicker(options=dateTimeOption),)
	employed_upto = forms.DateField(widget=DateTimePicker(options=dateTimeOption),)
	pf_number = forms.CharField(max_length=20, widget=forms.TextInput(attrs={'class': 'width-50 input-sm form-control',
		'required': 'False'}))
